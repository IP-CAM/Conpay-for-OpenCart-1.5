...
class ControllerModuleConpay extends Controller
{
	protected function index($setting)
	{
		...
		foreach ($results as $result)
		{
			...
			if ($p < $settings['min_price'] || $p > 200000)
			{
				continue;
			}
	...
	public function callback()
	{
		...
		if (!$settings)
		{
			$settings = array('merchant_id' => 0, 'api_key' => '');
		}
	...
	public function addorder()
	{
		...
		{
			$this->request->post['delivery'] = 0;
		}

		// Вычисляем контрольную сумму

		$parts = array($response_pass, is_numeric($post['delivery']) ? $total + $post['delivery'] : $total, $merchant_id,);
		...
		if ($post['checksum'] != $checksum || $_SERVER['HTTP_REFERER'] != 'https://www.conpay.ru' || $_SERVER['HTTP_USER_AGENT'] != 'Conpay')
		{
			die('Access denied.');
		}
