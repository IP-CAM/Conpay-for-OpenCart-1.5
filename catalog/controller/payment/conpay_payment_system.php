...
class ControllerPaymentConpayPaymentSystem extends Controller {
	...
	protected function index()
	{
		...
		if ($order_info)
		{
			...
			if ($v = html_entity_decode($order_info['payment_firstname'], ENT_QUOTES, 'UTF-8'))	$this->data['custom']['user_name'] = $v;
			if ($v = html_entity_decode($order_info['payment_lastname'], ENT_QUOTES, 'UTF-8')) $this->data['custom']['user_lastname'] = $v;
			if ($v = $order_info['email']) $this->data['custom']['user_email'] = $v;
